...
- `.firebaserc`의 기본 프로젝트를 `koofy-reader`로 지정했다. 서버와 보안 규칙 배포는 아직 하지 않았다.
...
- 운영 서버·규칙·관리자 배포와 IAM 서명 권한 설정은 아직 적용하지 않았다. 상세 흐름과 검증 결과는 [콘텐츠 관리자 문서](reader-content-admin.md)에 정리했다.
